...
const useAnimationLoop = (trackRef, targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical) => {
  ...
  useEffect(() => {
    ...
    const animate = timestamp => {
      ...
      if (seqSize > 0 && deltaTime < 0.1) { // Cap deltaTime to avoid jumps
        const currentVelocity = isHovered && hoverSpeed !== undefined ? hoverSpeed : targetVelocity;
        offsetRef.current = (offsetRef.current + currentVelocity * deltaTime) % seqSize;
...
export const LogoLoop = memo(
  ({
    ...
    // Initial and periodic measurement to ensure loop stays correct
    useEffect(() => {
      const timeout = setTimeout(updateDimensions, 100);
      return () => clearTimeout(timeout);
    }, [updateDimensions, logos.length]);
    ...
    const renderLogoItem = useCallback(
      (item, key) => {
        ...
        const content = isNodeItem ? (
          ...
          <img
            className={cx(
              'h-[var(--logoloop-logoHeight)] w-auto block object-contain grayscale invert',
