...
import { sanitizeInput, validateEmail, validateLength } from '../../utils/security';
...
export const Contact = () => {
  ...
  const handleManualSubmit = async (e) => {
    ...
    if (!formData.email || !validateEmail(formData.email)) {
      setSubmitStatus('Error: Please provide a valid email address.');
      setTimeout(() => setSubmitStatus(null), 5000);
      return;
    }
    
    if (!validateLength(formData.name, 100) || !validateLength(formData.message, 5000)) {
      setSubmitStatus('Error: Name or message exceeds maximum allowed length.');
      setTimeout(() => setSubmitStatus(null), 5000);
      return;
    }
  ...
  const handleGoogleSubmit = async () => {
    if (!formData.name || !formData.message) {
      setSubmitStatus('Error: Name and message are required before signing in.');
      setTimeout(() => setSubmitStatus(null), 5000);
      return;
    }
    ...
    try {
      ...
      // 2. Send data to backend service
      const response = await contactService.sendMessage({
        name: sanitizeInput(formData.name),
        email: userEmail,
        message: sanitizeInput(formData.message)
  ...
  return (
    <section id="contact" className="py-20 bg-neutral-950 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      <SEO 
        title="Contact | Kavya Patel"
        description="Get in touch with Kavya Patel for collaborations, freelance projects, or any web development opportunities."
      />
      {/* Background elements */}
        ...
        <div className="max-w-2xl mx-auto" ref={ref}>
          ...
          >
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${email}`} className="text-white font-medium hover:text-emerald-400 transition-colors break-all">
                      {email}
                    </a>
                  </div>
                </div>
            ...
            <div className="bg-gradient-to-r from-blue-900/40 to-emerald-900/40 border border-neutral-800 rounded-3xl p-8 flex items-center justify-between">
              ...
              >
                <Upload className="rotate-180" size={24} aria-hidden="true" />
              </a>
            </div>
          </motion.div>
            ...
            <form onSubmit={handleManualSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Kavya patel"
                />
              </div>

              <div>
                <label htmlFor="email" className="block text-sm font-medium text-neutral-400 mb-2">Your Email (Optional if using Google)</label>
                <input
                  type="email"
                  id="email"
                  name="email"
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-3 transition-all ${
                    isSubmitting 
                      ...
                      : 'bg-primary-500 text-white hover:bg-primary-600 cursor-pointer shadow-lg shadow-primary-500/20'
                  }`}
                >
